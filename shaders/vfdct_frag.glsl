...
const vec3 Cb_weights   = vec3(-0.1146, -0.3854, 0.5000); const vec3 Cr_weights   = vec3(0.5000, -0.4542, -0.0458);
...
void main()
{
        ...
        vec3 DCT = (sums *= (0.5 * Cy));

        float q_value = (20.0 * get_quant(int(local_uv.x), int(local_uv.y))) / 255.0; 
        vec3 quantized_channels = vec3(round(DCT / q_value));
